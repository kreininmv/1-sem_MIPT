...
#define DEF_INSTR(name, num, code)                     \
  case INSTR_##name:                                   \
    code                                               \
    printf("code COMMAND: (%i)   %i - %s (%lf)\n\n", num, cur_instr, #name, el);\
    return 1;

#define DEF_INSTRARG(name, arg, num, code)          \
  case INSTR_##name##arg:                           \
    code                                            \
    printf("code COMMAND: (%i)   %i - %s%s\n\n", num, cur_instr, #name, #arg); \
    return 1;
...
#define DEF_INSTRARGRAM(name, arg, num, code) \
  case INSTR_##name##arg##ram:                \
    code                                      \
    printf("code COMMAND: (%i)   %i - %s [%s+%lf]\n\n", num, cur_instr, #name, #arg, el); \
    return 1;
...
int mk5::proc::RInstFileAsme(const char* FileName)
{
  ...
  char Name[MAX_PATH] = "";
  strncat(Name, "..\\PROGRAMS\\", MAX_PATH);
