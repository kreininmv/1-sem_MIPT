...
{
  if ((element != '{' && element != '?' && element != ']' && element != '[' && element != '`') && (element < 'A' || element > 'z' || ('Z' < element && element < 'a')))
    return 1;
...
void PutPointsToStrings(const mk5::line buf, mk5::line **lines, int count_of_strings)
{
  ...
  for (i_sym = 0; i_sym < count_of_strings - 1; i_sym++)
  {
    ...
    cur_sym++;
    SkipTrash(&(*lines)[i_sym], FROM_BEGIN);
    
    //SkipTrash(&(*lines)[i_sym], FROM_BEGIN);

    (*lines)[i_sym].len = cur_sym - past_cur_sym + (*lines)[i_sym].len;
    if ((*lines)[i_sym].str[(*lines)[i_sym].len - 3] == '`' || (*lines)[i_sym].str[(*lines)[i_sym].len - 3] == '?')
      (*lines)[i_sym].str[(*lines)[i_sym].len - 3] = 0;
    if ((*lines)[i_sym].str[(*lines)[i_sym].len - 2] == '`' || (*lines)[i_sym].str[(*lines)[i_sym].len - 2] == '?')
      (*lines)[i_sym].str[(*lines)[i_sym].len - 2] = 0;
  }
